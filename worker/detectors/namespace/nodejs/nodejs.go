...
func detectNodejs(data map[string][]byte) *database.Namespace {
	for filename, _ := range data {
		...
		if match, _ := regexp.MatchString(NodejsRegexp, filename); match {
			fmt.Println("get node js namespace")
			//TODO: get nodejs version
			return &database.Namespace{Name: NodejsPkg + ":1.0"}
		}
	}
	return nil
}
...
func (detector *NodejsNamespaceDetector) GetRequiredFiles() []string {
	return []string{"^var/lib/rpm/Packages", "^var/lib/dpkg/status", NodejsRegexp}
}
